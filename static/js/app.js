document.addEventListener('DOMContentLoaded', () => {
    ...
    const getFaceImageSource = (face, matchedResult) => {
        ...
        if (matchedResult?.imported_image_url) return { kind: 'imported', src: matchedResult.imported_image_url };
        return source;
    };
    ...
    function isAutoGeneratedPersonName(name) {
        return /^人物\s*\d+$/.test(normalizePersonNameInput(name));
    }

    function isStorablePersonNameInput(name) {
        const normalized = normalizePersonNameInput(name);
        return Boolean(normalized) && !isAutoGeneratedPersonName(normalized);
    }

    function sortPersonNameMemory(items) {
        return [...items].sort((left, right) => {
            const usageDiff = Number(right?.usage_count || 0) - Number(left?.usage_count || 0);
            if (usageDiff !== 0) return usageDiff;
            const lastUsedDiff = String(right?.last_used_at || '').localeCompare(String(left?.last_used_at || ''));
            if (lastUsedDiff !== 0) return lastUsedDiff;
            const updatedDiff = String(right?.updated_at || '').localeCompare(String(left?.updated_at || ''));
            if (updatedDiff !== 0) return updatedDiff;
            return String(left?.display_name || '').localeCompare(String(right?.display_name || ''), 'zh-Hant');
        });
    }

    function ensurePersonNameSuggestionList() {
        let datalist = document.getElementById(PERSON_NAME_MEMORY_DATALIST_ID);
        if (datalist) return datalist;
        datalist = document.createElement('datalist');
        datalist.id = PERSON_NAME_MEMORY_DATALIST_ID;
        document.body.appendChild(datalist);
        return datalist;
    }

    function renderPersonNameSuggestionList() {
        const datalist = ensurePersonNameSuggestionList();
        datalist.innerHTML = sortPersonNameMemory(currentPersonNameMemory)
            .map(item => `<option value="${escapeHtml(item.display_name || '')}"></option>`)
            .join('');
    }

    function mergePersonNameMemoryItems(items) {
        const memoryMap = new Map(
            currentPersonNameMemory
                .filter(item => isStorablePersonNameInput(item?.display_name))
                .map(item => [normalizePersonNameInput(item.display_name), { ...item }]),
        );
        (items || []).forEach(item => {
            const displayName = normalizePersonNameInput(item?.display_name);
            if (!isStorablePersonNameInput(displayName)) return;
            const existing = memoryMap.get(displayName) || {};
            memoryMap.set(displayName, {
                display_name: displayName,
                normalized_name: displayName,
                source: item?.source || existing.source || 'manual',
                usage_count: Math.max(Number(item?.usage_count || 0), Number(existing?.usage_count || 0)),
                created_at: item?.created_at || existing.created_at || '',
                updated_at: item?.updated_at || existing.updated_at || '',
                last_used_at: item?.last_used_at || existing.last_used_at || '',
            });
        });
        currentPersonNameMemory = sortPersonNameMemory(Array.from(memoryMap.values()));
        renderPersonNameSuggestionList();
    }

    function rememberPersonNamesLocally(names, source = 'manual') {
        const now = new Date().toISOString();
        mergePersonNameMemoryItems(
            Array.from(new Set((names || []).map(normalizePersonNameInput).filter(isStorablePersonNameInput)))
                .map(displayName => ({
                    display_name: displayName,
                    normalized_name: displayName,
                    source,
    ...
    async function loadDriveNameMemory() {
        ...
        try {
            const response = await fetch('/drive/name-memory');
            const data = await response.json();
            if (!response.ok) throw new Error(data.detail || '人物名稱記憶載入失敗');
            currentPersonNameMemory = Array.isArray(data.items)
                ? sortPersonNameMemory(data.items.filter(item => isStorablePersonNameInput(item?.display_name)))
                : [];
        } catch (error) {
            console.warn('Drive name memory load failed:', error);
            currentPersonNameMemory = [];
        }
        renderPersonNameSuggestionList();
    }

    async function recordDriveNameMemory(names, source = 'manual') {
        const uniqueNames = Array.from(new Set((names || []).map(normalizePersonNameInput).filter(isStorablePersonNameInput)));
        if (uniqueNames.length === 0) return { success: true, skipped: true };
        rememberPersonNamesLocally(uniqueNames, source);
        if (!currentUser?.logged_in) return { success: true, skipped: true };
        try {
            const response = await fetch('/drive/name-memory/record', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ names: uniqueNames, source }),
            });
            const data = await response.json();
            if (!response.ok) throw new Error(data.detail || '人物名稱記憶儲存失敗');
            if (Array.isArray(data.items)) {
                mergePersonNameMemoryItems(data.items);
            }
            return data;
        } catch (error) {
            console.warn('Drive name memory record failed:', error);
            return { success: false, error: error.message };
        }
    }

    function collectCurrentPersonNames() {
        return currentFaceClusters
            .map(cluster => normalizePersonNameInput(cluster?.display_name))
            .filter(isStorablePersonNameInput);
    }

    async function persistCurrentPersonNames(source = 'manual') {
        const names = collectCurrentPersonNames();
        if (names.length === 0) return { success: true, skipped: true };
        return await recordDriveNameMemory(names, source);
    }
    ...
    function syncDriveSourceSelectionSummary() {
        ...
        if (driveFolderId?.value?.trim()) {
            const folderName = driveFolderId.dataset.folderName;
            driveSourceSelectionSummary.textContent = folderName ? `已選資料夾：${folderName}` : '已選雲端資料夾';
            return;
        }
        driveSourceSelectionSummary.textContent = '尚未選取雲端圖片或資料夾。';
    }
    ...
    function clearSelectedDriveFiles() {
        ...
        if (driveFolderId) {
            driveFolderId.value = '';
            delete driveFolderId.dataset.folderName;
        }
    ...
    analyzeBatchBtn.addEventListener('click', async () => {
        ...
        if (source === 'local') {
            ...
            const sourceFolderId = driveFolderId.value.trim();
            if (selectedDriveFiles.length === 0 && !sourceFolderId) {
                showToast('請先選 Google 雲端圖片或資料夾', 'error');
                return;
            }
            endpoint = '/batch_drive_start/';
            body = {
                folder_id: selectedDriveFiles.length ? null : sourceFolderId,
    ...
    async function mergeSelectedFaceClusters() {
        ...
        clearFaceMultiSelection();
        rememberPersonNamesLocally([merged.display_name], 'merge');
        persistBatchViewState();
        renderBatchOverview();
    ...
    async function moveSelectedEvidenceToNewCluster() {
        ...
        rememberPersonNamesLocally([newCluster.display_name], 'new_cluster');
        persistBatchViewState();
        renderBatchOverview();
            ...
            return `<article class="face-person-card${isExpanded ? ' expanded' : ''}${selectedPersonClass}">
                <div class="face-person-row">
                    ...
                    <div class="face-person-name-editor">
                        <input class="face-person-name-input" value="${escapeHtml(cluster.display_name)}" maxlength="80" aria-label="人物名稱" list="${PERSON_NAME_MEMORY_DATALIST_ID}" autocomplete="off">
    ...
    async function saveFaceClusterUpdate(clusterId, update) {
        ...
        const storableName = 'display_name' in update && isStorablePersonNameInput(cluster.display_name)
            ? cluster.display_name
            : '';
        if (storableName) rememberPersonNamesLocally([storableName], 'save_name');
        renderBatchOverview();
        try {
            ...
            if (storableName) {
                void recordDriveNameMemory([storableName], 'save_name');
            }
    ...
    async function saveBatchResultsToDrive() {
        ...
        try {
            await persistCurrentPersonNames('drive_export');
    ...
    function createPicker(targetId) {
        ...
        try {
            ...
            if (isSourcePicker) {
                ...
                view.setMimeTypes('image/jpeg,image/png,image/webp,image/gif,image/heic,image/heif,application/vnd.google-apps.folder');
            } else {
                view.setIncludeFolders(true);
                view.setSelectFolderEnabled(true);
            ...
            if (input) {
                if (targetId === 'drive-folder-id') {
                    if (folders.length > 0 && imageFiles.length > 0) {
                        showToast('請擇一選取：多張圖片，或一個資料夾', 'error');
                        return;
                    }
                    if (folders.length > 1) {
                        showToast('資料夾一次只能選一個', 'error');
                        return;
                    }
                    if (imageFiles.length === 0 && folders.length === 0) {
                    ...
                    if (folders.length === 1) {
                        ...
                        input.dataset.folderName = folder.name || folder.id;
                        input.dispatchEvent(new Event('change'));
                        syncDriveSourceSelectionSummary();
                        showToast(`已選取資料夾：${folder.name || folder.id}`);
                        pickedPayload = folder;
                    } else {
                        selectedDriveFiles = imageFiles;
                        input.value = '';
                        delete input.dataset.folderName;
                        input.dispatchEvent(new Event('change'));
                        syncDriveSourceSelectionSummary();
                        showToast(`已選取 ${imageFiles.length} 張雲端圖片`);
                        pickedPayload = imageFiles;
                    }
                } else {
                ...
                if (onPicked) onPicked(pickedPayload);
            }
