...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --max=3 \
  --cpu-throttling \
  --set-env-vars="GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID},GOOGLE_PROJECT_NUMBER=${GOOGLE_PROJECT_NUMBER:-225874268617},GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI},APP_BASE_URL=${APP_BASE_URL:-},PUBLIC_APP_ORIGIN=${PUBLIC_APP_ORIGIN:-},INSIGHT_API_URL=${INSIGHT_API_URL},PHOTOIDENTIFIER_EXPORTS_BUCKET=${PHOTOIDENTIFIER_EXPORTS_BUCKET:-photoidentifier-prod-exports},FIRESTORE_PROJECT_ID=${FIRESTORE_PROJECT_ID:-${PROJECT_ID}},FIRESTORE_DATABASE=${FIRESTORE_DATABASE:-(default)},BATCH_STATE_BACKEND=firestore,FACE_CLUSTERING_ENABLED=${FACE_CLUSTERING_ENABLED:-true},CLASSIFIER_STATUS_CACHE_TTL_SEC=${CLASSIFIER_STATUS_CACHE_TTL_SEC:-8},SESSION_HTTPS_ONLY=true,PREVIEW_SIGNED_URL_TTL_MINUTES=${PREVIEW_SIGNED_URL_TTL_MINUTES:-1440},EXPORT_SIGNED_URL_TTL_MINUTES=${EXPORT_SIGNED_URL_TTL_MINUTES:-60},ADMIN_GOOGLE_USER_IDS=${ADMIN_GOOGLE_USER_IDS:-}" \
  --set-secrets="SESSION_SECRET=SESSION_SECRET:latest,GOOGLE_CLIENT_SECRET=GOOGLE_CLIENT_SECRET:latest,INSIGHT_API_KEY=INSIGHT_API_KEY:latest,VERTEX_API_KEY=VERTEX_API_KEY:latest,PHOTOIDENTIFIER_BACKEND_SERVICE_ACCOUNT_JSON=PHOTOIDENTIFIER_BACKEND_SERVICE_ACCOUNT_JSON:latest"
